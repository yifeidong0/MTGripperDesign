...
class UPush(Task):
    ...
    def get_obs(self) -> np.ndarray:
        ...
        # observation = np.concatenate([object_rotation, task_int, object_rad, self.random_target]) # Debugging
    ...
    def is_success(self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info: Dict[str, Any] = {}) -> np.ndarray:
        # return self.is_success_flag # Debugging
    ...
    def _eval_robustness(self, tool_positions, tool_angles, object_positions, design_params, object_rad,
                         is_inside_gripper, slack=0.0):
        """
        ...
        assert design_params.shape == (N, 4) and object_positions.shape == (N, 2) and tool_positions.shape == (
        N, 2) and tool_angles.shape == (N, 1)
        a1 = design_params[:, 0]
        l1 = design_params[:, 1]
        a2 = design_params[:, 2]
        l2 = design_params[:, 3]
    ...
    def compute_reward(
        ...
        # Ensure inputs are 2D arrays for consistency
    ...
    #     object_rad = observation[..., 9].reshape(-1, 1)
        ...
        # object_target_yaw = np.arctan2(desired_goal[..., 1] - achieved_goal[..., 1],
        #                                desired_goal[..., 0] - achieved_goal[..., 0])
        # ee_object_yaw = np.arctan2(achieved_goal[..., 1] - ee_position_2d[..., 1],
        #                            achieved_goal[..., 0] - ee_position_2d[..., 0])
        # ee_target_yaw = np.arctan2(desired_goal[..., 1] - ee_position_2d[..., 1],
        #                            desired_goal[..., 0] - ee_position_2d[..., 0])
        # yaw_difference_ee_object = abs(pi_2_pi(ee_yaw - np.pi / 2 - ee_object_yaw))
        # yaw_difference_ee_target = abs(pi_2_pi(ee_yaw - np.pi / 2 - ee_target_yaw))

        # reward = 0
        # reward += -(1*ee_object_distance + 2*yaw_difference_ee_object)
        
        # if ee_object_distance < 0.15 and yaw_difference_ee_object < 0.1:
        #     reward += -(5*object_target_distance)
