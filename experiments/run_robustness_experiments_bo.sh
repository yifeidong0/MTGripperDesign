...
for i in {1..6}; do
  ...
  if [ ${#model_files[@]} -ne 4 ]; then
    echo "Error: Expected 4 zip files in results/paper/catch/$i/"
    exit 1
  fi

  # Generate csv_filename paths based on the required format
  timestamp=$(date +%Y%m%d_%H%M%S)
  csv_file_a="results/paper/catch/$i/catch_bo_results_${timestamp}_1_1.csv"
  csv_file_b="results/paper/catch/$i/catch_bo_results_${timestamp}_0_1.csv"
  # csv_file_c="results/paper/catch/$i/catch_bo_results_${timestamp}_1_0.csv"
  # csv_file_d="results/paper/catch/$i/catch_bo_results_${timestamp}_0_0.csv"
