...
class Box2DSimulationEnv(gym.Env):
    def __init__(self, v_angle, object_type='circle', gui=True, img_size=(84, 84)):
        ...
        self.action_space = spaces.Box(low=np.array([-0.1, -0.1, -0.01]), high=np.array([0.1, 0.1, 0.01]), dtype=np.float32)
        ...
        self.goal_position = np.array([random.uniform(-20, 20), random.uniform(0, 50)])
    ...
    def reset(self, seed=None, options=None):
        ...
        self.simulation.robot_body.position = (random.uniform(-25, 25), random.uniform(0, 55))

        # Ensure object does not penetrate gripper: place it at least some distance away from the gripper
        min_distance = 5  # Minimum distance to ensure no penetration
        while True:
            obj_x = random.uniform(-25, 25)
            obj_y = random.uniform(0, 55)
    ...
    def _compute_reward(self):
        ...
        return -distance_to_goal if distance_to_goal > self.goal_radius else 1000
    ...
    def _is_truncated(self):
        ...
        # Check if the gripper is out of the canvas
        ...
        return bool(time_ended or gripper_out_of_canvas)
